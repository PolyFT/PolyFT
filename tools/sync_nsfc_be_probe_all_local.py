#!/usr/bin/env python3
"""Synchronize the cumulative NSFC B/E probe release to local storage."""
...
DEFAULT_TAG = "nsfc-be-probe-all-1019-2026-08-27"
DEFAULT_ASSET = "nsfc-be-probe-all-1019.tar.gz"


def request_json(url: str, token: str | None) -> dict[str, Any]:
    headers = {
        "Accept": "application/vnd.github+json",
        "User-Agent": "PolyFT/nsfc-be-probe-all-sync/1.0",
...
def download(url: str, path: Path, token: str | None) -> None:
    headers = {"User-Agent": "PolyFT/nsfc-be-probe-all-sync/1.0"}
    if token:
        headers["Authorization"] = f"Bearer {token}"
    with urllib.request.urlopen(
...
def main() -> None:
    ...
    parser.add_argument("--dest", default="data-local/nsfc-be-probes/all-1019")
    parser.add_argument(
        "--token",
    ...
    with tempfile.TemporaryDirectory(prefix="nsfc-probe-all-", dir=str(dest.parent)) as temp_dir:
        temp = Path(temp_dir)
